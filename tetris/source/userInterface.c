...
extern const u_char pieceColor[7][3];
  ...
  for (int counter_c = 0; counter_c < F_COLS; counter_c++) {
    for (int counter_r = 0; counter_r < F_ROWS; counter_r++) {
      ...
      if (whatIsInside(field[counter_c][counter_r]) == PLAYER) {
        u_char piece = field[counter_c][counter_r] - 1;
        filledRect(x1, y1, x2, y2, pieceColor[piece][0],
                                   pieceColor[piece][1],
                                   pieceColor[piece][2]);
        //rect(x1, y1, x2, y2, 26, 26, 26);
      } else if (whatIsInside(field[counter_c][counter_r]) == FILLED) {
        u_char piece = (field[counter_c][counter_r] / 100) - 1;
        filledRect(x1, y1, x2, y2, setPieceColor[piece][0],
                                   setPieceColor[piece][1],

...
#else // raw input control, from main

#include "config.h" // keySettings
	
	/* custom pad control from egl.c */
//int x, y;
/*	for (unsigned int i = 0; i < 8; i++)
	{ keysactive[keySettings[i]] = 0; }
*/
	int ret = updateController(&keysactive);
	//int ret = updateController();
	if (ret)
	{
		lastkey_scan = ret;
		lastkey_mod  = KMOD_NONE;
		// hook our custom input buttons to keyboard
		//keysactive[ret] = true;
		newkey       = true;
		keydown 	 = true;
		debugNetPrintf(INFO, "updateController:%d\n", ret);
	} else
	{   // clean all input (4 directions + 4 buttons)
		keydown = false;
		
	}
	//debugNetPrintf(INFO, "ret:%d\n", ret);

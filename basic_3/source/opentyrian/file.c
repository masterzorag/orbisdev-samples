...
FILE *dir_fopen_warn(  const char *dir, const char *file, const char *mode )
{
	...
	if (f == NULL)
		fprintf(ERROR, "warning: failed to open '%s': %s\n", file, strerror(errno));
...
FILE *dir_fopen_die( const char *dir, const char *file, const char *mode )
{
	...
	if (f == NULL)
	{
		fprintf(ERROR, "error: failed to open '%s': %s\n", file, strerror(errno));
		fprintf(ERROR, "error: One or more of the required Tyrian " TYRIAN_VERSION " data files could not be found.\n"
...
size_t efread( void *buffer, size_t size, size_t num, FILE *stream )
{
	...
	if (num_read != num)
	{
		fprintf(ERROR, "error: An unexpected problem occurred while reading from a file.\n");
		JE_tyrianHalt(1);
	}
...
size_t efwrite( const void *buffer, size_t size, size_t num, FILE *stream )
{
	...
	if (num_written != num)
	{
		fprintf(ERROR, "error: An unexpected problem occurred while writing to a file.\n");
		JE_tyrianHalt(1);
	}

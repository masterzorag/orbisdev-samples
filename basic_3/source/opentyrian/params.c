...
void JE_paramCheck( int argc, char *argv[] )
{
	...
	for (; ; )
	{
		...
		case OPTION_MISSING_ARG:
			fprintf(ERROR, "Try `%s --help' for more information.\n", argv[0]);
			exit(EXIT_FAILURE);
			break;
			
		case 'h':
			fprintf(INFO, "Usage: %s [OPTION...]\n\n"
			       "Options:\n"
		...
		case 'n':
			...
			if (temp)
			{
				...
				{
					fprintf(ERROR, "%s: error: invalid network port number\n", argv[0]);
					exit(EXIT_FAILURE);
				}
		...
		case 257: // --net-player-number
		{
			...
			{
				fprintf(ERROR, "%s: error: invalid network player number\n", argv[0]);
				exit(EXIT_FAILURE);
			}
			break;
		}
		...
		case 'd':
		{
			...
			{
				fprintf(ERROR, "%s: error: invalid network delay value\n", argv[0]);
				exit(EXIT_FAILURE);
			}
			break;
		}
